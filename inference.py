...
Two modes:
  --mode openai   (DEFAULT) — Uses OpenAI API.
...
def log_start(task_id: str, episode: int = 0):
    print(f'[START] {json.dumps({"task": task_id, "episode": episode})}', flush=True)


def log_step(task_id: str, step: int, action_type: str, reward: float):
    print(f'[STEP]  {json.dumps({"task": task_id, "step": step, "action": action_type, "reward": round(reward, 4)})}', flush=True)


def log_end(task_id: str, episode: int, score: float, steps: int):
    print(f'[END]   {json.dumps({"task": task_id, "episode": episode, "score": round(score, 4), "steps": steps})}', flush=True)
...
def run_task_rule_based(task_id: str, episode: int = 0, verbose: bool = False) -> dict:
    ...
    with httpx.Client(timeout=30, base_url=BASE_URL) as client:
        ...
        while not obs.get("done", False) and steps < max_steps:
            ...
            if section_idx == prev_section_idx:
                ...
                steps += 1
                log_step(task_id, steps, "submit", result.get("reward", 0.0))
                break
            ...
            if task_id == "clause_identification":
                ...
                obs = result.get("observation", obs)
                steps += 1
                log_step(task_id, steps, "identify_clause", result.get("reward", 0.0))
                ...
                if not obs.get("done", False):
                    ...
                    obs = result.get("observation", obs)
                    steps += 1
                    log_step(task_id, steps, "next_section", result.get("reward", 0.0))
                ...
                if has_risk:
                    ...
                    obs = result.get("observation", obs)
                    steps += 1
                    log_step(task_id, steps, "flag_risk", result.get("reward", 0.0))
                    ...
                    if not obs.get("done", False):
                        ...
                        obs = result.get("observation", obs)
                        steps += 1
                        log_step(task_id, steps, "assess_severity", result.get("reward", 0.0))
                ...
                if has_change:
                    ...
                    obs = result.get("observation", obs)
                    steps += 1
                    log_step(task_id, steps, "detect_change", result.get("reward", 0.0))
                    ...
                    if not obs.get("done", False):
                        ...
                        obs = result.get("observation", obs)
                        steps += 1
                        log_step(task_id, steps, "assess_impact", result.get("reward", 0.0))
        ...
        resp.raise_for_status()
        grade = resp.json()
        log_end(task_id, episode, grade.get("score", 0.0), steps)
...
def run_task_openai(task_id: str, episode: int = 0, verbose: bool = False) -> dict:
    """Run a task using OpenAI API."""
    print(f"\n{'=' * 55}")
    ...
    with httpx.Client(timeout=30, base_url=BASE_URL) as client:
        ...
        while not obs.get("done", False) and steps < max_steps:
            ...
            obs = result.get("observation", obs)
            steps += 1
            log_step(task_id, steps, action_dict.get("action_type", "unknown"), result.get("reward", 0.0))
...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, default="openai", choices=["rule", "openai"],
        help="Agent mode: 'rule' (free) or 'openai' (uses API). Default: openai",
    )
    ...
    print(f"\n  Mode: {'Rule-Based (FREE)' if args.mode == 'rule' else f'OpenAI ({MODEL_NAME})'}")

    results = []
    for tid in tasks:
        try:
            results.append(run_fn(tid, episode=args.episode, verbose=args.verbose))
